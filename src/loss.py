...
class coral_loss(nn.Module):
    ...
    def forward(self, source, target):
        ...
        # Reshape the tensors
        source = source.view(-1, d)
        target = target.view(-1, d)

        # Compute the covariance matrices
        source_cov = (1 / (source.shape[0] - 1)) * torch.mm(source.t(), source)
        target_cov = (1 / (target.shape[0] - 1)) * torch.mm(target.t(), target)

        # Compute the Frobenius norm between source and target covariance matrices
